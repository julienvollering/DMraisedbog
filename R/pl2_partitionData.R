...
run_clara_clustering <- function(data, top_features, k_clusters, target_col,
  ...
  clara_result <- clara(
    ...
    # 10e3 is about 34% of data, SAC means nearby points are similar.
    # Need to capture full spatial variation.
    sampsize = min(10e3, nrow(clustering_data) * 0.1),
    trace = 1
  )
...
## Map visualization ####
mf_current_with_coords_outer <- mf_current_with_coords |>
  select(response, x, y) |>
  mutate(
...
train_data <- mf_current[train_idx, ]
...
## Map visualization ####
data_with_coords_inner <- mf_current_with_coords[train_idx, ] |>
  select(response, x, y) |>
  mutate(
...
inner_folds_full <- rep(NA_integer_, nrow(mf_current))
...
writeRaster(raster_outer, "output/pl2/partition_outer.tif", overwrite = TRUE)
writeRaster(raster_inner, "output/pl2/partition_inner.tif", overwrite = TRUE)
