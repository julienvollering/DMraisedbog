...
query_res <- 500
template_query <- rast(europe_ext_3035, resolution = query_res, crs = "EPSG:3035")
...
predictors_current <- fill_threshold_na(
  predictors_current, THRESHOLD_VARS, "unified model - current"
)
predictors_future <- fill_threshold_na(
  predictors_future, THRESHOLD_VARS, "unified model - future"
)
...
# Clean up
rm(
  chelsa_past_stack, chelsa_past_cropped, chelsa_past_3035, chelsa_past_masked,
  chelsa_future_stack, chelsa_future_cropped, chelsa_future_3035, chelsa_future_masked,
  paleo_stack, paleo_cropped, paleo_3035, paleo_masked,
  elev_raw, elev_3035,
