...
most_shifted_w_sample <- w_sample_results |>
  slice_max(W_sample, n = 1) |>
  pull(W_sample)

cat("\nMost shifted feature (highest W_sample):", most_shifted_feature, "\n")
cat("W_sample =", round(most_shifted_w_sample, 4), "\n")

## Functions ####
source("R/functions.R")

## Pre-compute prediction distances on most shifted feature ####

cat("\nPre-computing prediction distances on", most_shifted_feature, "...\n")

precomputed_distances <- calculate_prediction_distances(
  training_data = mf_current,
  prediction_data = mf_future_presence,
  variable_name = most_shifted_feature,
...
feature_weights <- setNames(1, most_shifted_feature)

# Check for existing evaluation results
eval_file <- "output/pl2/n_cuts_evaluation_maxshift.csv"
